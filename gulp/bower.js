...
gulp.task('bower:scripts', ['bower:partials'], function() {
  return gulp.src([
    path.join(paths.src, '/app/staffingTracker/**/*.js'),
    path.join(paths.tmp, 'partials', 'templateCacheHtml.js')
  ])
    .pipe($.angularFilesort())
    .pipe($.ngAnnotate())
    .pipe($.concat('staffingTracker.js'))
...
gulp.task('bower:styles', function() {
  return gulp.src(path.join(paths.src, '/app/staffingTracker/staffingTracker.scss'))
...
gulp.task('bower:partials', function () {
  return gulp.src([
    paths.src + '/app/staffingTracker/**/*.html'
  ])
    .pipe($.minifyHtml({
      empty: true,
      spare: true,
